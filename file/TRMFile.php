...
/**
 * класс для работы с файлом - запись данных в файл из буфера, 
 * чтение данных в буфер из файла, данные представлены в виде строки
 * 
 */
class TRMFile extends TRMState
{
  ...
  public function setFileName($name)
  {
    if (is_string($name)) {
      ...
      $this->StateString = "передано неверное имя файла name";
      return false;
    }
  ...
  public function setFileDir($name)
  {
    if (is_string($name)) {
      ...
      $this->StateString = "передано неверное директории name";
      return false;
    }
  ...
  /**
   * устанавливает полное имя файла, 
   * разбирает на части и сохраняем в отдельных переменных имя и путь 
   * 
  ...
  {
    if (empty($fullpath)) {
      $this->StateString = "передан пустой fullpath";
      return false;
    }
  ...
  /**
   * помещает данные в буфер, приводятся к строке
   * 
   * @param string $buffer - данные для сохранения в буфере
   */
  ...
  /**
   * @return string - буфер, содержащий прочитанные данные, 
   * или подготовленные для записи в файл
   */
  ...
  public function openFile($filename = "", $mod = 'r')
  {
    ...
    if (!empty($filename)) {
      if (!$this->setFullPath($filename)) {
        return false;
      }
    }

    if (!$this->FullPath) {
      $this->StateString = "FullPath не задан";
      return false;
    }
    ...
    if (!$this->Handle) {
      $this->StateString = "Файл открыть не удалось";
      return false;
    }
  ...
  public function getAllFileToBuffer($filename = "")
  {
    ...
    if (!is_file($filename)) {
      $this->StateString = "Имя файла указано не верно {$filename}";
      return false;
    }
    $this->clearBuffer();
    $this->Buffer = file_get_contents($filename);
    if ($this->Buffer === false) {
      $this->StateString = "Не удалось прочитать содержимое файла {$filename}";
      return false;
    }
   ...
   * @return bool - если файл прочитан удачно, то вернется true,
   * иначе false, также если файл не был открыт вернется false
   */
  public function getStringToBufferFrom($length = 4096)
  {
    if (!$this->Handle) {
      $this->StateString = "файл не открыт";
      return false;
    }

    $str = fgets($this->Handle, $length);
    if (!$str) {
      $this->StateString = "не удалось прочитать очередную строку";
      return false;
    }
   ...
   * 
   * @param int $length - количество байт, которое будет считано из файла
   * @return bool - если файл прояитан удачно, то вернется true,
   * иначе false, также если файл не был открыт вернется false
   */
  public function getToBuffer($length = 8192)
  {
    ...
    if (!$str) {
      $this->StateString = "не удалось прочитать {$length} байт из файла";
      return false;
    }
  ...
  {
    if (empty($string)) {
      $this->StateString = "пустая строка";
      return 0;
    }
  ...
  public function existFile($filename = "")
  {
    ...
    if (is_file($filename) === false) {
      $this->StateString = "файл не найден {$filename}";
      return false;
    }
   ...
   * 
   * @return bool - TRUE - если указатель файла находится вконце, либо файл не открыт, FALSE если указатель файла еще не достиг конца
   */
   ...
   *
   * @return int|bool - возвращает размер загруженных данных в байтах, либо false в случае неудачного открытия потока.
