...
use TRMEngine\Image\Exceptions\TRMImageExceptions;
...
class TRMImage
{
  ...
  /**
   * @var resource|GdImage - ресурс исходного рисунка
   */
  public $SrcImage;
  /**
   * @var resource|GdImage - ресурс для формируемого рисунка
   */
  public $DestImage;
  ...
  public function getImageFromFile($filename)
  {
    ...
    switch ($this->ImageType) {
      case IMAGETYPE_BMP:
        $this->SrcImage = imagecreatefrombmp($filename);
        break;
      case IMAGETYPE_GIF:
      ...
      case IMAGETYPE_PNG:
        ...
        break;
      case IMAGETYPE_WBMP:
        $this->SrcImage = imagecreatefromwbmp($filename);
        break;
      default:
   ...
   * @return bool - в случае упеха true, если сформировать не удалось - false
   */
  ...
  public function generateNewSizeImage($MaxW = 0, $MaxH = 0, $ImageType = IMAGETYPE_JPEG)
  {
    ...
    if ($this->Height > $this->Width) {
      $sizecoeffx = $this->Width / $this->Height;
    }
    ...
    // для вычисления коэффицента маштабирования высоты результирующего изображения
      ...
      $sizecoeffy = $this->Height / $this->Width;
    }
  ...
  public function saveDestImageToFile($filename)
  {
    ...
    switch ($this->DestImageType) {
      ...
      case IMAGETYPE_GIF:
        ...
        break;
      case IMAGETYPE_PNG:
        if (!$this->DestImageQuality) {
          $this->DestImageQuality = 7;
        }
  ...
  function saveSrcImageToFile($filename)
  {
    ...
    switch ($this->ImageType) {
      ...
      case IMAGETYPE_PNG:
        if (!$this->DestImageQuality) {
          $this->DestImageQuality = self::DEFAULT_PNG_QUALITY;
        }
      ...
      default:
        if (!$this->DestImageQuality) {
          $this->DestImageQuality = self::DEFAULT_JPEG_QUALITY;
        }
