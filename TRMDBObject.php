...
class TRMDBObject
{
   ...
   * 
   * @return bool - в случае успеха вернет true
  ...
  public function connect()
  {
    ...
    while (!$this->newlink) {
      ...
      $this->newlink = new \mysqli(
        isset($this->ConfigArray["dbserver"]) ? $this->ConfigArray["dbserver"] : null,
   ...
   * 
   * @return bool - true в случае успешного соединения с БД , иначе функциЯ connect выбрасывает исключение \Exception
   */
   ...
   *  
   * @return bool - true в случае успешного соединения с БД , 
  ...
  public function multiQuery($querystring)
  {
    ...
    do {
      ...
      if ($this->newlink->insert_id) {
        $ResArr->setRow("insert_id", $this->newlink->insert_id);
      }
