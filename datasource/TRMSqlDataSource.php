...
class TRMSqlDataSource extends TRMState implements TRMDataSourceInterface
{
    ...
    foreach ($SafetyFields as $CurrentTableName => $CurrentTable) {
      foreach ($CurrentTable as $CurrentFieldName => $CurrentField) {
        ...
        if (!empty($CurrentField->Relation)) {
          $JoinedTables[$CurrentField->Relation[TRMDataMapper::OBJECT_NAME_INDEX]][TRMDataMapper::FIELDS_INDEX][$CurrentField->Relation[TRMDataMapper::FIELD_NAME_INDEX]]
            = array(
          ...
          if (!empty($CurrentTable->Alias)) {
            $JoinedTables[$CurrentField->Relation[TRMDataMapper::OBJECT_NAME_INDEX]]["ObjectAlias"] = $CurrentTable->Alias;
          }

          $JoinedTables[$CurrentField->Relation[TRMDataMapper::OBJECT_NAME_INDEX]]["Join"] =  isset($CurrentField->Relation["Join"]) ?
  ...
  public function generateJoinStringForTable($TableName, array &$TableState)
  {
    ...
    if (!empty($TableState["ObjectAlias"])) {
      $joinstr .= $TableState["ObjectAlias"];
    }
    $joinstr .= " ON ";
  ...
  protected function generateConditionString(array &$Params)
  {
    ...
    foreach ($Params as $TableName => $TableParams) {
      foreach ($TableParams as $param) {
        $key = $param["key"];
        if (isset($param["alias"]) && strlen($param["alias"]) > 0) {
          $key = $param["alias"] . "." . $key;
        } else if (!empty($TableName)) {
          $key = $TableName . "." . $key;
        }
        if (isset($param["quote"]) && $param["quote"] == TRMSqlDataSource::NEED_QUOTE) {
        ...
        if ($param["operator"] == "IN" || $param["operator"] == "NOT IN") {
          ...
          $wherestr .= "'" . addcslashes(trim($param["value"], "'"), "'") . "' ";
        }
      }
    }
  ...
  public function makeSelectQuery(TRMSafetyFields $SafetyFields)
  {
    ...
    if (is_int($this->Count)) {
      $this->QueryString .= " LIMIT {$this->Count}";
    }
    if (is_int($this->StartPosition)) {
      $this->QueryString .= " OFFSET {$this->StartPosition}";
    }
  ...
  protected function generateInsertQueryString(
    ...
    if (!($TableCount = $DataObject->count())) {
      return;
    }
