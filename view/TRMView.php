...
class TRMView
{
  ...
  /**
   *
   * @var bool - эсли установлен в true, 
  ...
  public function __construct($view = "", $layout = "")
  {
    ...
    if (null === $layout) {
      ...
      $this->LayoutName = strlen($layout) > 0 ? strtolower(trim($layout)) : self::DefaultLayoutName;
    }
  }
  ...
  public function __toString()
  {
    ...
    if (null === $this->LayoutName) {
      ...
      if (!is_file($LayoutFile)) {
        return "";
      } // throw new \Exception( "Не найден макет {$LayoutFile}!" ); }
      ob_start();
      require $viewfilepath;
      // сохраняем вывод
      ...
      if ($this->ScriptsToEndFlag) {
        $this->cutScripts();
        if (false === strpos($this->FullContent, "</body>")) {
          $this->FullContent .= $this->Scripts;
        } else {
          $this->FullContent = str_replace("</body>", $this->Scripts . "</body>", $this->FullContent);
        }
      }
    } // else = if layout
   ...
   * @param bool $top - если true, то добавляется в раздел head - в начало страницы, иначе будет добавляться перед закрытием тега /body
   */
   ...
   * 
   * @param bool $topflag = если true, то выводятся все скрипты с установленным влагом top
   */
