...
class TRMEMail
{
   ...
   * 
   * @return bool - в случае успеха true, иначе исключение
  ...
  public function sendEmail()
  {
    ...
    if (strlen($msg) > 0 && $this->ContentType == "multipart/mixed") {
      $msg = "--" . $this->un . "\r\n" . $msg;
    }
  ...
  protected function generateHeader()
  {
    ...
    if (!empty($this->emailCc)) {
      if (empty($this->nameCc)) {
        $this->nameCc = $this->emailCc;
      }
   ...
   * 
   * @return string|bool - если удалось определить тип содержимого письма, 
  ...
  protected function calculateContentType()
  {
    ...
    if (strlen($this->texthtml) && strlen($this->textplain)) {
      return "multipart/alternative";
    }
    if (strlen($this->texthtml) &&  !empty($this->inlineimages)) {
      return "multipart/related";
    }
    if (strlen($this->texthtml)) {
      return "text/html";
    }
   ...
   * 
   * @param bool $strength - указывает строгая проверуа, или же нет и тело письма вместе с темой можно оставлять пустывми
   * @return bool
   */
  ...
  {
    if (is_string($name)) {
      $this->namefrom = $this->setStrCharset(strip_tags($name));
    }
  }
  ...
  {
    if (is_string($name)) {
      $this->nameto = $this->setStrCharset(strip_tags($name));
    }
  }
  ...
  public function setReplyTo($email, $name)
  {
    ...
    if (is_string($name)) {
      $this->namereplyto = $this->setStrCharset(strip_tags($name));
    }
  }
  ...
  {
    if (is_array($msg)) {
      $this->texthtml = $this->setStrCharset(implode(' ', $msg));
    } else if (is_string($msg)) {
      $this->texthtml = $this->setStrCharset($msg);
    }
  }
  /**
  ...
  {
    if (is_array($msg)) {
      $this->textplain = $this->setStrCharset(strip_tags(implode(' ', $msg)));
    } else if (is_string($msg)) {
      $this->textplain = $this->setStrCharset(strip_tags($msg));
    }
  }
  /**
  ...
  {
    if (is_file($filename)) {
      return $this->filestosend[] = $filename;
    }
    return false;
  }
  ...
  {
    if (is_file($filename)) {
      return $this->inlineimages[] = $filename;
    }
    return false;
  }
  ...
  public function setConfigArray(array $ConfigArray)
  {
    ...
    if (!is_array($this->config) || empty($this->config)) {
      throw new TRMEMailExceptions(__METHOD__ . " Файл конфигурации вернул неверный формат данных!");
    }

    if (isset($this->config["emailto"])) {
      $this->setEmailTo($this->config["emailto"]);
    }
    if (isset($this->config["nameto"])) {
      $this->setNameTo($this->config["nameto"]);
    }
    if (isset($this->config["cc"])) {
      $this->setEmailCc($this->config["cc"]);
    }

    if (isset($this->config["smtpemailfrom"])) {
      $this->setEmailFrom($this->config["smtpemailfrom"]);
    }
    if (isset($this->config["smtpnamefrom"])) {
      $this->setNameFrom($this->config["smtpnamefrom"]);
    }
    if (isset($this->config["charset"])) {
      $this->setCoding($this->config["charset"]);
    }
   ...
   *
   * @return bool - в случае отправки вернет true, иначе
   * @throws TRMEMailSendingExceptions
   */
   ...
   * @return bool - если в сокете найдена строка с первыми символами кода $response, а затем пробел, то возвращается true, иначе false
   */
